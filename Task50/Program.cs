...
int[,] array2D = GenerateMatrix(3, 4, -100, 100);
Console.WriteLine("Введите номер строки");
int i = Convert.ToInt32(Console.ReadLine()) - 1;
Console.WriteLine("Введите номер столбца");
int j = Convert.ToInt32(Console.ReadLine()) - 1;
PrintMatrix(array2D);
if (PresenceOfAnElementInAnArray(array2D, i, j)) Console.WriteLine($" {i}, {j} -> Такого элемента в массиве нет");
else Console.WriteLine($" {i + 1}, {j + 1} -> {array2D[i, j]}");
